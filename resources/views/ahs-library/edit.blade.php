                    ...
                    <div x-data="ahsForm()">
                        <form method="POST" action="{{ route('ahs-library.update', $ahs_library) }}">
                            ...
                            <div>
                                ...
                                <textarea id="notes" name="notes" rows="2" class="block mt-1 w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm">{{ old('notes', $ahs_library->notes) }}</textarea>
                            </div>
                                ...
                                <template x-for="(material, index) in materials" :key="`mat-${index}`">
                                    <div class="flex items-center space-x-2 p-2 bg-gray-50 rounded border" x-init="initializeTomSelect($el.querySelector('.tom-select-materials'))">
                                        <div class="flex-1">
                                            ...
                                            <select x-model="material.inventory_item_id" @change="updateMaterialCost(index, $event)" :id="`mat_id_${index}`" :name="`materials[${index}][inventory_item_id]`" class="tom-select-materials block mt-1 w-full border-gray-300 text-sm rounded-md shadow-sm" required>
