<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            ...
            <a href="{{ route('item-categories.create') }}" class="inline-flex items-center px-4 py-2 bg-blue-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-blue-700 active:bg-blue-800 focus:outline-none focus:border-blue-800 focus:ring ring-blue-300 disabled:opacity-25 transition ease-in-out duration-150">
                                ...
                                @forelse ($categories as $category)
                                    <tr class="hover:bg-gray-50 transition duration-150">
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium space-x-2">
                                            <a href="{{ route('item-categories.edit', $category) }}" class="text-indigo-600 hover:text-indigo-800" title="Edit">
                                            ...
                                            <form class="inline-block" action="{{ route('item-categories.destroy', $category) }}" method="POST" onsubmit="return confirm('Are you sure?');">
                                                @csrf
                                                @method('DELETE')
