<x-app-layout>
    <x-slot name="breadcrumbs">
        <x-breadcrumbs :items="[
            ['label' => 'Inventory', 'url' => route('inventory-items.index')],
            ['label' => 'New Item', 'url' => '']
        ]" />
    </x-slot>
                ...
                <div class="p-6 text-gray-900">
                    <form method="POST" action="{{ route('inventory-items.store') }}">
                        ...
                        <p class="text-sm text-gray-600 mb-4">
                            The **Item Code** will be automatically generated based on the category you select.
                        </p>
                        ...
                        <div class="mt-4">
                            ...
                            <p class="mt-1 text-xs text-gray-500">e.g., "pcs", "m3", "kg", "unit"</p>
                        </div>
                        
                        <div class="mt-4">
                            <x-input-label for="base_purchase_price" :value="__('Base Purchase Price (Optional)')" />
                            <x-text-input id="base_purchase_price" class="block mt-1 w-full" type="number" step="0.01" name="base_purchase_price" :value="old('base_purchase_price', 0)" />
                            <x-input-error :messages="$errors->get('base_purchase_price')" class="mt-2" />
                            <p class="mt-1 text-xs text-gray-500">Default cost reference for new Purchase Orders.</p>
                        </div>

                        <div class="mt-4">
                            <x-input-label for="reorder_level" :value="__('Reorder Level')" />
                            <x-text-input id="reorder_level" class="block mt-1 w-full" type="number" step="1" name="reorder_level" :value="old('reorder_level', 0)" />
                            <x-input-error :messages="$errors->get('reorder_level')" class="mt-2" />
                            <p class="mt-1 text-xs text-gray-500">Alert level for low stock reports.</p>
                        </div>
