    ...
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">
                {{ __('Work Items Library') }}
            </h2>
            
            <a href="{{ route('work-items.create') }}" class="inline-flex items-center px-4 py-2 bg-blue-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-blue-700 active:bg-blue-800 focus:outline-none focus:border-blue-800 focus:ring ring-blue-300 disabled:opacity-25 transition ease-in-out duration-150">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 mr-1" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                  <path stroke-linecap="round" stroke-linejoin="round" d="M12 4v16m8-8H4" />
                </svg>
                {{ __('Add New Work Item') }}
            </a>
        </div>
    </x-slot>
                                ...
                                @forelse ($workItems as $item)
                                    <tr class="hover:bg-gray-50 transition duration-150">
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600">
                                            {{-- $item->workType->name ?? 'N/A' --}}
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium space-x-2">
                                            <a href="{{ route('work-items.edit', $item) }}" class="text-indigo-600 hover:text-indigo-800" title="Edit / Manage AHS">
                                            ...
                                            <form class="inline-block" action="{{ route('work-items.destroy', $item) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete this Work Item?');">
                                                @csrf
                                                @method('DELETE')
