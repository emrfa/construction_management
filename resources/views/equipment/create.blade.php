<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Add New Equipment Type/Record') }} {{-- Adjusted Title --}}
        </h2>
    </x-slot>
        ...
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    ...
                    <div x-data="{ status: '{{ old('status', 'pending_acquisition') }}' }"> {{-- Default to pending --}}
                        <form method="POST" action="{{ route('equipment.store') }}">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                                ...
                                <div>
                                    <x-input-label for="identifier" :value="__('Asset Code (Optional/Base)')" />
                                    <x-text-input id="identifier" class="block mt-1 w-full" type="text" name="identifier" :value="old('identifier')" placeholder="e.g., EXC-001 (Final code set during receiving)" />
                                    <p class="text-xs text-gray-500 mt-1">Unique code. Can be finalized during PO receiving.</p>
                                    <x-input-error :messages="$errors->get('identifier')" class="mt-2" />
                                </div>

                                <div>
                                    <x-input-label for="type" :value="__('Type / Category (Optional)')" />
                                    <x-text-input id="type" class="block mt-1 w-full" type="text" name="type" :value="old('type')" placeholder="e.g., Heavy Machinery, Power Tool"/>
                                ...
                                <div>
                                    ...
                                    <select id="status" name="status" x-model="status" class="block mt-1 w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm" required>
                                        {{-- User mainly creates records before acquisition or records rentals --}}
                            ...
                            <div x-show="status === 'rented'" x-transition class="mt-6 border-t pt-6">
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                                    ...
                                    <div>
                                        ...
                                        <x-input-error :messages="$errors->get('rental_start_date')" class="mt-2" />
                                    </div>
                                    {{-- Rental End Date --}}
                                    <div>
                                        ...
                                        <x-input-error :messages="$errors->get('rental_end_date')" class="mt-2" />
                                    </div>
                                    {{-- Rental Rate --}}
                                    <div class="flex items-end space-x-2">
                                        ...
                                        <div class="w-24">
                                             ...
                                             <select id="rental_rate_unit" name="rental_rate_unit" class="block mt-1 w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm">
                                              ...
                                              <x-input-error :messages="$errors->get('rental_rate_unit')" class="mt-2" />
                                        </div>
                                    </div>
                                     {{-- Agreement Ref --}}
                                    <div class="md:col-span-3">
                                        ...
                                        <x-input-error :messages="$errors->get('rental_agreement_ref')" class="mt-2" />
                                    </div>
                                </div>
                            </div>

                            {{-- *** ADDED BASE PRICING SECTION *** --}}
                            <div class="mt-6 border-t pt-6">
                                 <h3 class="text-lg font-medium text-gray-900 mb-4">Reference Pricing (Optional)</h3>
                                 <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                     <div>
                                         <x-input-label for="base_purchase_price" :value="__('Typical Purchase Price (Rp)')" />
                                     ...
                                     <div class="flex items-end space-x-2">
                                         <div class="flex-1">
                                            <x-input-label for="base_rental_rate" :value="__('Typical Rental Rate (Rp)')" />
