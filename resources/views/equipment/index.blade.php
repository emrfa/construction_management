<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <div>
                <h1 class="text-2xl font-bold text-gray-900">Equipment Master</h1>
                <p class="text-sm text-gray-500 mt-1">Manage owned and rented equipment assets.</p>
            </div>
                ...
                <form method="GET" action="{{ route('equipment.index') }}">
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                        ...
                        <div class="flex items-end gap-2">
                            ...
                            <a href="{{ route('equipment.index') }}" class="px-4 py-2 bg-white border rounded-xl text-sm text-gray-700 hover:bg-gray-50 shadow-sm">Clear</a>
                        </div>
                    </div>
                </form>
            </div>
                        ...
                        <tbody class="bg-white divide-y divide-gray-100">
                            @forelse ($equipmentItems as $item)
                                <tr class="hover:bg-gray-50/60 transition">
                                    <td class="px-6 py-4 whitespace-nowrap text-sm font-mono text-gray-700">{{ $item->identifier ?? 'N/A' }}</td>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full
                                            @switch($item->status)
                                                @case('owned') bg-green-100 text-green-800 @break
                                                @case('rented') bg-blue-100 text-blue-800 @break
                                                @case('maintenance') bg-yellow-100 text-yellow-800 @break
                                                @case('disposed') bg-gray-200 text-gray-600 @break
                                                @default bg-gray-100 text-gray-600 @break
                                            @endswitch">
                                            {{ ucfirst($item->status) }}
                                        </span>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600">{{ $item->supplier->name ?? '-' }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium flex justify-end gap-3">
                                        <a href="{{ route('equipment.edit', $item) }}" class="text-gray-500 hover:text-indigo-600" title="Edit">
                            ...
                            @empty
                                <tr><td colspan="6" class="py-6 text-center text-gray-500 text-sm">No equipment found.</td></tr>
                            @endforelse
                        </tbody>
                    </table>
                </div>
                <div class="px-6 py-4 border-t bg-gray-50">{{ $equipmentItems->appends(request()->query())->links() }}</div>
            </div>
            
            <x-modal name="confirm-delete" focusable>
                <form method="post" x-bind:action="`/equipment/${itemId}`" class="p-6">
                    @csrf @method('delete')
                    <h2 class="text-lg font-medium text-gray-900">{{ __('Delete Equipment?') }}</h2>
